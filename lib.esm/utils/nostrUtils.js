...
// Npub to hex.
// One address.
export const convertNpubOrHexAddressToHex = (npubNoteNeventHex) => {
...
// Hex to npub, note.
export const convertHexToBech32 = (hexKey, 
...
// Hex to note.
// One address.
export const convertHexNoteNeventIdToNote = (id) => {
    if (!id)
        return "";
    if (typeof (id) !== "string")
        return "";
    ...
    id.startsWith("nevent") &&
        id.length === 68) {
        idNote = convertHexToBech32(convertNpubOrHexAddressToHex(id), "note");
    }
    else if (
    // Id is already note
    id.startsWith("note") &&
...
// Multiple addresses.
export const convertHexNoteNeventIdsToNote = (idsHexNoteNevent) => {
    const arrayOfIdsNote = [];
    if (!hasValue(idsHexNoteNevent))
...
//   } else { return ""}
// }
export const convertValueToNostrTagsMapping = (value, algorithm) => {
    let newValue = "";
    let method = "";
    let original = "";
    if (!value)
...
export const toBeNostrHex = (value, algorithm, length = 64) => {
    try {
        if (!String(value))
            return null;
        const valueStr = String(value);
        const map = convertValueToNostrTagsMapping(valueStr, algorithm);
        if (!map)
            return null;
